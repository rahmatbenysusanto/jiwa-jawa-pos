    ...
    <div class="page-header">
        <div class="add-item d-flex">
            <div class="page-title">
                <h4 class="fw-bold">Edit Recipe Menu</h4>
            </div>
        </div>
        <div class="page-btn">
                    ...
                    <div class="row">
                        <div class="col-6">
                            <div class="mb-3">
                                ...
                                <select class="form-control" id="menu" disabled>
                                    <option value="{{ $recipeMenu->id }}">{{ $recipeMenu->name }}</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-6">
...
@section('js')
    <script>
        ...
        function loadExistingData() {
            localStorage.setItem('menu', JSON.stringify(@json($recipeMenu)));

            const basicMaterials = @json($basicMaterials);
            const materialBasicMenu = [];
            basicMaterials.forEach((item) => {
            ...
            const variantMaterialsDb = @json($variantMaterials);
            const variantMap = {};
            variantMaterialsDb.forEach((item) => {
            ...
            const variant = Object.values(variantMap);
        ...
        function createRecipe() {
            Swal.fire({
                ...
                if (i.value) {
                    ...
                    $.ajax({
                        url: '{{ route('menu.recipe.menu.update') }}',
                        method: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
